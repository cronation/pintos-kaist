...
// P3
// pml4e_walk 호출 없이 pte를 직접 인자로 받음
void pml4_pte_set_dirty(uint64_t *pml4, uint64_t *pte, const void *vpage, bool dirty) {
	ASSERT(pte != NULL);
	if (dirty)
		*pte |= PTE_D;
	else
...
// P3
// pml4e_walk 호출 없이 pte를 직접 인자로 받음
void pml4_pte_set_accessed(uint64_t *pml4, uint64_t *pte, const void *vpage, bool accessed) {
	ASSERT(pte != NULL);
	if (accessed)
		*pte |= PTE_A;
	else
