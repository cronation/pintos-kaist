...
// P1-AS
#define FRAC (1 << 14)
// from int to 17.14 format
#define TO_REAL(x) ((x) * FRAC)
// from 17.14 format to int
#define TO_INT(x) ( (x) > 0 ? (((x) + FRAC/2) / FRAC) : (((x) - FRAC/2) / FRAC) )
...
thread_init (void) {
	...
	if (thread_mlfqs) { // P1-AS
		initial_thread->nice = 0; // 초기 쓰레드 nice는 0
		list_init(&all_list);
